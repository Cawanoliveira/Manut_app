    ...
    container.innerHTML = visibleItems.map(function(item) {
      ...
      if (isDias) {
        return '<div class="config-item">' +
          ...
          '<div><span class="config-fixed-value">10 dias</span><input type="hidden" class="config-input" data-chave="' + escapeHtml(item.chave) + '" value="10"></div>' +
        '</div>';
      }
      return '<div class="config-item">' +
        '<div><strong>' + escapeHtml(formatarNomeConfiguracao_(item.chave)) + ':</strong></div>' +
        '<div><input type="' + inputType + '" class="config-input" data-chave="' + escapeHtml(item.chave) + '" value="' + escapeHtml(inputValue) + '"></div>' +
      '</div>';
    }).join('');
  }
